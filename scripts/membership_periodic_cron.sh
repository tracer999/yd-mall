#!/usr/bin/env bash
# 정기(월) 쿠폰 발급 크론 진입점 (설계 §7.1, 2차). 매월 1일 실행.
#   crontab 예: 0 9 1 * * /data/yd-mall/scripts/membership_periodic_cron.sh
# ENCRYPTION_KEY 로드 · nvm node · flock 중복방지 · 항상 exit 0.
set -uo pipefail
ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
cd "$ROOT" || exit 0
LOG_DIR="$ROOT/logs"; LOG="$LOG_DIR/membership_periodic.log"; mkdir -p "$LOG_DIR"
...
LOCK="$ROOT/logs/.membership_periodic.lock"
if command -v flock >/dev/null 2>&1; then
  exec 9>"$LOCK"
  flock -n 9 || { log "이미 실행 중 — 건너뜀"; exit 0; }
fi

node scripts/calc_membership_periodic.js >> "$LOG" 2>&1
rc=$?
[ $rc -ne 0 ] && log "✗ 정기 쿠폰 발급 실패 (exit $rc)"
exit 0
