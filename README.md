...
**이 저장소는 dev-mall 단독 프로젝트입니다.** 서브프로젝트나 워크스페이스가 없고, `app.js` 가 곧 진입점입니다.

상품·카테고리·재고·이미지의 **소스 오브 트루스(Source of Truth)** 는 이 앱의 DB(`dev_mall` @ MySQL)입니다. 여기에 더해, 국내몰을 원천으로 삼아 해외향 Shopify 스토어(`ydatasvcmall.myshopify.com`)를 여는 시나리오의 **연동 코드가 관리자에 내장**되어 있습니다.
...
>
> 과거 이 Shopify 스토어를 소비하던 헤드리스 고객몰과 자체 관리자는 **별도 저장소**로, 이 저장소에는 없습니다. dev-mall 과는 코드 의존 없이 Shopify 스토어를 매개로 간접 연결됩니다.
...
|------|------|------|
| Runtime | Node.js | **22** (운영 서버는 `dev-mall.sh` 가 nvm 으로 선택) |
...
운영 서버에서는 `dev-mall.sh` 를 씁니다. `/etc/environment` 에서 `ENCRYPTION_KEY` 를 직접 읽고 nvm 22 를 로드하므로, PAM 을 거치지 않는 CI/CD SSH 세션에서도 동작합니다.

```bash
./dev-mall.sh build          # npm install + Tailwind 빌드
./dev-mall.sh start          # PM2 기동/갱신 (상용)
./dev-mall.sh start dev      # 개발 모드로 기동
./dev-mall.sh status | logs | restart | stop | delete
```
...
**개발 DB 와 운영 DB 가 같습니다**(`dev_mall` @ ydata.co.kr). 로컬 검증 스크립트가 쓴 테스트 행도 운영 데이터에 그대로 들어갑니다.
