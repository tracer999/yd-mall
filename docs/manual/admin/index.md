...
4. 로그인에 성공하면 **접근 권한이 있는 첫 번째 메뉴**(보통 **몰 리스트 관리**)로 이동합니다. 대시보드가 첫 화면이 아닙니다.
...
>
> 다만 **카테고리 · 브랜드 · 포인트 · 운영자 계정은 전 몰이 공유하는 전역 데이터**입니다. 한 몰에서 바꾸면 모든 몰에 함께 적용됩니다. (카테고리·브랜드는 각 몰에 상품이 있는 것만 노출되고 몰별 숨김만 개별 조절됩니다 — [카테고리 관리](/manual/admin/categories) · [브랜드 관리](/manual/admin/brands) 참고.)
...
> ⚠️ 이 그룹은 **7개 화면이 모두 동작**합니다(공급처/채널 연결, 상품 가져오기, 가져온 상품, 스마트스토어 등록, 스토어 상품 가져오기, 재고·주문 가져오기, 네이버 리소스 관리). 다만 **재고·주문 가져오기** 화면은 현재 **재고 전송(우리 몰 → 네이버, 수동)만** 제공하고 주문 수집·발주·송장 반영은 다음 차수 과제입니다. 상태 표는 문서를 보세요.

### 3. 메뉴/카테고리 관리
| 메뉴 | 설명 | 문서 |
|------|------|------|
| 카테고리 | **상품 분류**(최대 3단계) 추가·수정·삭제 (사용중/전체 탭) | [categories.md](/manual/admin/categories) |
| 일반 메뉴 관리 | 고객 화면 상단 메뉴 켜기/끄기·이름·순서 | [menus.md](/manual/admin/menus) |
| 브랜드 관리 | **브랜드 전체**(등록·삭제·순서·로고·소개·집계 재계산) | [brands.md](/manual/admin/brands) |
...
| 거래처 할인 | 거래처마다 얹어 줄 추가 할인율 | [b2b.md](/manual/admin/b2b) |
| B2B 주문 | 기업 주문 승인·입금확인·**출고**·세금계산서 (**결제 없이 접수**) | [b2b.md](/manual/admin/b2b) |
| B2B 클레임 | 기업 주문의 취소·반품 승인과 **계좌 환불** 마감 | [b2b.md](/manual/admin/b2b) |
| 견적 관리 | 대량 주문 단가 협상·견적서 발행·주문 전환 | [b2b.md](/manual/admin/b2b) |
| B2B 설정 | 입금 기한·계좌 안내·세액 표기 등 공통 설정 | [b2b.md](/manual/admin/b2b) |

> 상품별 **B2B 할인율·최소 주문수량**은 이 그룹이 아니라 **상품 등록/수정 화면 안**의 `B2B(사업자) 판매` 섹션에서 정합니다.
>
> ⚠️ 기업 주문을 **승인하면 입금 전이라도 재고가 빠집니다.** 미입금 주문은 B2B 주문 화면에서 회수해야 합니다.
>
> 📌 **기업 주문은 이 그룹에서만 처리합니다.** 접수 → 승인 → 입금확인 → 출고 → 취소·반품까지 전부 여기서 끝납니다. 아래 「주문/회원 관리」의 판매 관리·배송 관리·클레임 관리에는 **일반 회원 주문만** 나옵니다.

### 9. 주문/회원 관리
| 메뉴 | 설명 | 문서 |
|------|------|------|
| 판매 관리 | 주문 목록·상세·상태 변경 (**일반 회원 주문만**) | [sales.md](/manual/admin/sales) |
| 배송 관리 · 배송비 정책 | 송장 입력, 배송 완료 처리, 배송비 기준 (**일반 회원 주문만**) | [shipping.md](/manual/admin/shipping) |
| 클레임 관리 | 취소·반품 승인/거절, 환불 (**일반 회원 주문만**) | [claims.md](/manual/admin/claims) |
| 회원 관리 | 회원 목록·상세·비활성화 | [users.md](/manual/admin/users) |

> 기업(사업자) 주문은 이 그룹에 **나오지 않습니다.** 위 「8. B2B 관리」에서 처리하세요.
...
| 샘플 데이터 관리 | 몰 생성 시 복제되는 원본 카테고리·브랜드·상품·히어로 | [service.md](/manual/admin/service) |
