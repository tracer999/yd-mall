...
그 옵션으로 만들 수 있는 상품의 **재고가 0이거나 판매중지(OFF)** 상태입니다. 옵션이 두 종류 이상이면(예: 색상+사이즈) **이미 고른 옵션에 맞춰 다시 계산**되므로, 색상을 바꾸면 품절 표시가 달라질 수 있습니다. 재고는 <span class="manual-path">상품 관리 &gt; 옵션 관리</span> 에서 확인·수정합니다.
