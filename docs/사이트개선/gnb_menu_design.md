...
### 2-1. 오늘특가 `/deal/today` — ✅ 완료 (관리자 수동 큐레이션)

> ## ⚠️ 이 절의 설계는 폐기됐다 (2026-07-11)
> 아래 카운트다운·`DEADLINE_SALE` 뱃지 자동 노출 설계는 **구현했다가 되돌렸다.**
> 사용자 확정: **오늘특가는 관리자가 상품그룹에서 지정한다.** 뱃지 자동 필터가 아니다.
>
> **실제 구현**: 몰별 오늘특가 manual 상품그룹(`seed_key='ct_deal'`) → `getList({groupId})` 로 렌더.
> 홈 '오늘의 특가' 캐러셀과 **같은 그룹을 공유**한다. 0건이면 준비중 랜딩 폴백.
> **카운트다운 없음** — 수동 큐레이션에는 만료일 개념이 없다.
> 관리는 `/admin/product-groups`. 아래 원문은 이력으로만 남긴다.
...
### 2-2. 아울렛 `/outlet` — ⬜ 폐기 → 별도 모듈로 이관

> ## ⚠️ 이 절의 설계는 폐기됐다 (2026-07-11)
> **아울렛은 `discount_rate` 필터 목록이 아니다.** 사용자 확정:
> *"아울렛은 필터에 의한 상품이 아니고 몰내 몰임. 즉 몰 내 아울렛 상품 전용으로 구성을 하는 메뉴임.
> 따라서 관리자 메뉴에서 아울렛 관리 메뉴가 따로 있어야 함. 단순 상품 그룹으로만 관리하면 약함."*
>
> **아울렛 전용 상품 + 자체 카테고리 + 전용 관리자 메뉴**를 갖는 기획전급 신규 모듈이다.
> 아래 할인율 구간 필터 설계는 구현했다가 **되돌렸다**.
>
> → **설계·결정사항: [`outlet_design_and_development.md`](./outlet_design_and_development.md)**
> 현재 `/outlet` 은 준비중 랜딩 유지. 아래 원문은 이력으로만 남긴다.
...
### 2-3. 베스트 `/best` — ✅ 완료 (관리자 수동 큐레이션)

> ## ⚠️ 이 절의 '조회수 상위 100' 설계는 보조 폴백으로 강등됐다 (2026-07-11)
> 사용자 확정: **"베스트는 조회수 배치 등으로 자동 지정이 아닌 관리자 수동 지정임."**
>
> 조사 결과 **홈 베스트 그리드는 이미 관리자 수동 지정**(`product_group` id=1, manual)이었고
> **GNB `/best` 만 조회수 자동**이라 둘이 서로 다른 상품을 보여주고 있었다.
>
> **실제 구현**: 몰별 '베스트' manual 상품그룹을 소스로 렌더(홈 그리드와 동일 그룹).
> 수동 지정이 **0건인 몰만** 조회수 상위 100 자동 폴백(빈 화면 방지) — 상품을 담는 순간 수동 모드로 전환.
> 관리는 `/admin/product-groups`.
...
> ## ⚠️ 갱신 (2026-07-11) — 오늘특가·베스트는 **관리자 수동 큐레이션**으로 재정의됐다
>
> 사용자 확정: **"오늘특가는 관리자가 상품그룹에서 지정하여 보여주는 것"**, **"베스트도 자동 지정이 아닌 관리자 수동 지정"**.
> 조회수·뱃지 자동 방식은 폐기했다. 아래 L1·L2·L4 는 그 결과로 다시 쓴 것이다.
>
> **구현 방식**: `productController.getList` 에 **`groupId` 프리셋**을 추가해
> `product_group_item`(관리자 수동 매핑)만 조회한다. 별도 뷰·컨트롤러를 만들지 않는다.
> 관리자 큐레이션 순서(`product_group_item.sort_order`)를 존중한다.
> **홈 섹션과 같은 상품그룹을 공유**하므로 관리자가 한 곳(`/admin/product-groups`)만 관리하면
> 홈·GNB 가 함께 바뀐다.
>
> **스캐폴드(S1) 추출은 하지 않았다.** `views/user/products/list.ejs` 가 이미 전체상품·카테고리·브랜드·
> 신상품·베스트·오늘특가를 모두 렌더하는 단일 공유 뷰라, S1 이 막으려던 '같은 화면 다섯 벌'이
> 애초에 발생하지 않았다. 메뉴별 차이는 **조건부 include** 로 처리한다.

- [x] **L1** 신상품 `/new` — `preset({badge:'NEW'})`. NEW 뱃지 상품만 (mall1 7 / mall2 200)
- [x] **L2** 베스트 `/best` — **관리자 수동 지정 우선**(몰별 '베스트' manual 상품그룹) → 홈 베스트 그리드와 동일 그룹.
      수동 지정이 **0건인 몰만** 조회수 상위 100 자동 폴백(빈 화면 방지). 상품을 담는 순간 수동 모드로 전환된다.
      실측: mall1 수동 4건 / mall2 수동 50건(BEST 뱃지 중 무작위 매핑)
- [ ] ~~**L3** 아울렛 `/outlet` — `minDiscount` 필터~~ → **폐기.** 아울렛은 필터 목록이 아니라
      **몰 안의 몰**(전용 상품 + 자체 카테고리 + 전용 관리자 메뉴)이다.
      → **[`outlet_design_and_development.md`](./outlet_design_and_development.md)** 로 이관. 현재 준비중 랜딩 유지.
- [x] **L4** 오늘특가 `/deal/today` — **관리자 수동 지정**(`seed_key='ct_deal'` manual 상품그룹) → 홈 '오늘의 특가'
      캐러셀과 동일 그룹. 담긴 상품이 0건이면 준비중 랜딩 폴백.
      **카운트다운은 제거**했다(수동 큐레이션에는 만료일 개념이 없다 — 사용자 확정).
      부수 효과: 만료된 특가가 홈에 계속 노출되던 버그가 함께 해소됐다.
- [ ] **L5** 랭킹 `/ranking` — 조회수 실시간 + 카테고리 탭 (`ranking_tabs` 승격)

### 8-4. 나머지 메뉴

- [ ] **M1** 브랜드 `/brands` — 초성 필터 + 페이지네이션 (mall=2 1,354개)
- [x] **M2** 멤버십 `/membership` — 제도 소개 정적 페이지 (안 A). 4등급 카드 + 혜택 3블록 + 적립금 링크.
      등급 산정·`user_grade` 없음(2차)
