...
**신상품 `/new` — "새로 들어온 것"**
전체를 최신순 정렬하는 것이 아니라 `FIND_IN_SET('NEW', product_badge)` 로 **자른다**(§7 결정 2). 신상품이 0건이면 "최근 입고 없음"을 보여주는 게 정직하다.

> 기간 필터(`created_at >= NOW() - INTERVAL 30 DAY`)를 쓰지 않는 이유는 §2-4 참고 — 시드 데이터의 `created_at` 이 전부 같아 30일 필터가 "전부 아니면 전무"로 튄다.
...
**계약 (잠정 — 확정하지 않는다)**
```js
// 스캐폴드가 받는 locals — 두 번째 목록 메뉴를 붙일 때 귀납해서 확정한다
...
> **이 계약을 먼저 못 박지 않는다 (2026-07-10 결정).**
> 다섯 메뉴는 **디스플레이 형태**가 비슷할 뿐, 기능이 갈릴 수 있다. 사례 하나로 추상화를 결정하면
> 나중에 억지로 끼워 맞추게 된다. 따라서 **중복이 확정된 것만 먼저 빼고, 나머지는 두 번째 사례에서 추출한다.**
>
> | 구분 | 대상 | 근거 |
> |---|---|---|
> | **지금 추출** | 정렬바 · 페이지네이션 · 빈 상태 | 다섯 메뉴 어디서도 형태가 달라질 이유가 없다 |
> | **보류** | `index.ejs` 골격, 슬롯 A/B/C 계약 | 두 번째 메뉴를 붙이며 실제 공통부를 확인한 뒤 확정 |
>
> **랭킹은 스캐폴드 밖일 수 있다.** §2-5 와이어프레임은 `4위 [카드] ▲2` 형태의 **세로 리스트**이지
> 상품 그리드가 아니다. 카드 바깥에 순위·변동이 붙고 1·2·3위는 포디움이다.
> **베스트도 총건수를 100으로 캡**해야 해서 페이지네이션 계산이 일반 목록과 다르다.
> 두 메뉴 모두 스캐폴드 적용 여부를 **구현 시점에 다시 판단**한다.
...
| 선행 | 2-0 의 정렬바·페이지네이션 파티셜(S1). **골격은 재사용하지 않을 수 있다** — 위 와이어프레임은 그리드가 아니라 세로 순위 리스트다. 적용 여부는 구현 시 판단 |
...
user_coupons   issued_by enum 에 'DOWNLOAD' 추가  (현재 AUTO/ADMIN/CODE)
               + UNIQUE (user_id, coupon_id)       ← 폐기 (아래 참고)
```

> ~~`UNIQUE (user_id, coupon_id)` 는 **바로 걸 수 있다**~~ — **폐기 (2026-07-10 개정).**
> 우려했던 "같은 쿠폰을 두 번 지급해야 하는 운영 요구"가 **이미 코드에 존재한다**:
> `admin/couponController.js:259-262` 의 중복 검사는 `used_at IS NULL` 기준이라 **사용한 쿠폰의 재발급을
> 허용**한다. 전역 UNIQUE 는 이 경로를 DB 레벨에서 깨뜨린다.
> 대신 다운로드 전용 `coupon_download(user_id, coupon_id)` PK 로 1인 1회 수령만 막는다.
> → [`coupon_design_and_development.md`](./coupon_design_and_development.md) §6-3
...
-- affectedRows = 0 → 마감/종료. 아니면 user_coupons INSERT.
-- 중복 수령 차단은 coupon_download(user_id, coupon_id) PK 로 한다 (이관 문서 §6-3).
```

#### 개발 계획

> **이 표는 이관 문서 §10-2-1 (D1~D13) 이 대체한다.** 아래는 초안으로 남긴다.

| 단계 | 내용 |
|---|---|
| 0 | **선행: B2** — 마이페이지 쿠폰함이 없는 컬럼을 조회해 항상 비어 있었다 (✅ `98985fc` 수정) |
| 1 | `coupons` ALTER + `user_coupons.issued_by` enum 확장 + `coupon_download` 테이블 (UNIQUE 제약은 폐기) |
...
| 4 | `POST /coupon/:id/claim` (트랜잭션 + `coupon_download` PK + 선착순) |
...
| B2 | 마이페이지 쿠폰함이 없는 컬럼 조회 | `controllers/mypageController.js:52,263-271` | **쿠폰** | `.catch` 폴백으로 항상 빈 쿠폰함 | ✅ 수정됨 (`98985fc`) |
| B3 | `badge_expire_date` 미적용 | `controllers/productController.js` DEADLINE_SALE 분기 | **오늘특가** | 만료 3주 지난 상품(id 106)이 특가에 노출 | ✅ 수정됨 (`98985fc`) |
| B4 | `HEADER_CS` 가 `/cs` 가 아닌 `/boards/notice` | `feature_menu` 데이터 | **이벤트&혜택** | 고객센터·이벤트가 같은 화면을 가리킴 | ✅ 수정됨 (`12f25f6`, DB) |

B2·B3 는 코드 수정, B4 는 데이터 수정이었다. **4건 모두 완료**(§6 0차, §8-0 참고).
...
이 셋은 중복이 확정돼 있다. 건너뛰면 같은 코드를 다섯 번 쓰게 된다.

골격(index.ejs)과 슬롯 계약은 여기서 만들지 않는다.
두 번째 목록 메뉴를 붙이며 실제 공통부를 확인한 뒤 확정한다(§2-0, §8-2 S3).
랭킹(세로 순위 리스트)·베스트(100건 캡)는 스캐폴드 밖일 수 있다.
```
...
쿠폰          coupons ALTER + 다운로드 수령(issued_by='DOWNLOAD') + coupon_download PK
              → 선행: 관리자에서 다운로드 쿠폰 등록(현재 마스터 3건뿐)
              → 상세: coupon_design_and_development.md (UNIQUE(user_id,coupon_id) 는 폐기)
```
...
> **개정 2026-07-10 — 이 절은 [`coupon_design_and_development.md`](./coupon_design_and_development.md) 가 대체한다.**
> `UNIQUE (user_id, coupon_id)` 는 **폐기**한다. `admin/couponController.js:259-262` 의 중복 검사가
> `used_at IS NULL` 기준이라 **사용한 쿠폰의 재발급을 허용**하는데, 전역 UNIQUE 는 이를 DB 레벨에서 깨뜨린다.
> 대신 **다운로드 전용 `coupon_download(user_id, coupon_id)` PK** 로 1인 1회 수령만 제약한다(이관 문서 §6-3).
> 아래 원문은 이력으로 남긴다.
...
제약        UNIQUE (user_id, coupon_id)  — 중복 수령 방지. 현재 이 제약이 없다   ← 폐기됨
...
2. 성격이 비슷한 목록형 메뉴(오늘특가·아울렛·베스트·신상품·랭킹)는 **중복이 확정된 파티셜(§2-0 의 정렬바·페이지네이션·빈상태)을 먼저 뽑은 뒤** 처리한다.
   골격과 슬롯 계약은 선(先)확정하지 않는다 — 다섯 메뉴는 **표시 형태가 비슷할 뿐 기능이 갈릴 수 있으므로**, 각 메뉴 구현 시 공통 여부를 다시 확인한다.
...
목록형 5개 메뉴의 선행 작업. **골격을 먼저 설계해 메뉴를 끼워 넣는 방식이 아니라, 중복이 확정된 것만 먼저 빼고 나머지는 두 번째 사례에서 추출한다**(§2-0 의 '계약(잠정)' 참고).

- [ ] **S1** `views/user/products/list.ejs` 에서 **정렬바 · 페이지네이션 · 빈상태**만 파티셜로 추출
      (`index.ejs` 골격은 이 단계에서 만들지 않는다)
- [ ] **S2** 추출 후 기존 `/products` 가 동일하게 렌더되는지 확인 (리팩터링 회귀 검증 — 생략 불가)
- [ ] **S3** 두 번째 목록 메뉴 구현 시 슬롯 계약(hero / filters / cardOpts / empty)을 **귀납해서 확정**
      → 첫 메뉴(신상품)는 `/products` 와 사실상 같아 계약이 드러나지 않는다.
      → **랭킹은 세로 순위 리스트, 베스트는 100건 캡**이라 스캐폴드 밖에 둘 수 있다. 구현 시 판단.
...
- [ ] **M3** 쿠폰 `/coupon` — 다운로드 수령. **설계·체크리스트는 [`coupon_design_and_development.md`](./coupon_design_and_development.md) §10-2 로 이관**
