...
├─ ② 이번 주 브랜드 혜택 ★  (슬라이더 — 쿠폰·기획전·특가·공동구매)
├─ ③ 인기 브랜드            (전체 / 카테고리별 / 급상승)
├─ ④ 신규·주목 브랜드        (입점일 기준)
├─ ⑤ 카테고리별 브랜드       (패션·뷰티·리빙…)
└─ ⑥ 전체 브랜드            (초성 인덱스 + 정렬 + 페이지네이션)

★ 혜택은 검색 바로 아래 **첫 콘텐츠 블록**이다. 브랜드 허브에 오는 고객의 상당수는
  "지금 뭐가 싼가"를 먼저 보고, 그 다음에 브랜드를 고른다. 목록보다 행사가 위다.
...
> **혜택 슬라이더는 별도 입력 화면이 없다.** 운영자가 평소대로 기획전·쿠폰·특가·공동구매를
> 만들면 `benefitService.getWeeklyBenefits()` 가 `brand_stat.benefit_count > 0` 인 브랜드를
> 혜택 많은 순으로 뽑아 **브랜드당 대표 혜택 1건**씩 슬라이드로 만든다(우선순위: 기획전 →
> 특가 → 공동구매 → 쿠폰). 브랜드당 1건인 이유는, 혜택이 많은 브랜드 하나가 슬라이더를
> 독점하면 "여러 브랜드가 행사 중"이라는 정보 자체가 사라지기 때문이다.
>
> 슬라이드 이미지는 기획전·공동구매의 `list_thumbnail_url`, 쿠폰의 `thumbnail_url` 을 쓰고,
> 특가처럼 이미지가 없는 혜택은 그 브랜드의 **대표 상품 이미지**(`brand_stat.rep_product_ids`)로
> 채운다. 새 혜택이 슬라이더에 안 보이면 `benefit_count` 가 집계 캐시라서이며,
> 관리자 브랜드 화면의 **집계 재계산** 또는 `scripts/recalc_brand_stat.js` 로 갱신한다.
...
├────────────────────────────────────────────────────────┤
│  이번 주 브랜드 혜택                          ‹  ›       │
│  진행 중인 브랜드 기획전·특가·쿠폰·공동구매                │
│  ┌──────────────┐ ┌──────────────┐ ┌──────────────┐    │
│  │ [기획전] D-26│ │ [특가]  D-60 │ │ [쿠폰]  D-7  │    │
│  │   ▣ 이미지   │ │   ▣ 이미지   │ │   ▣ 이미지   │    │
│  ├──────────────┤ ├──────────────┤ ├──────────────┤    │
│  │ ◍ 더엣지     │ │ ◍ 셀렙샵     │ │ ◍ 백세식품   │    │
│  │ 브랜드 위크  │ │ 타임특가     │ │ 3,000원 할인 │    │
│  └──────────────┘ └──────────────┘ └──────────────┘    │
│  ← scroll-snap 슬라이더. 5초 자동 넘김(hover 시 정지)     │
├────────────────────────────────────────────────────────┤
│  인기 브랜드                          [전체] [카테고리별] │
│  ┌────┐ ┌────┐ ┌────┐ ┌────┐ ┌────┐ ┌────┐            │
│  │ 1  │ │ 2  │ │ 3  │ │ 4  │ │ 5  │ │ 6  │  → 가로스크롤 │
...
| `views/user/brands/_benefit_slider.ejs` | 이번 주 브랜드 혜택 슬라이더 (scroll-snap, 자동 넘김) |
| `views/user/brands/_benefits.ejs` | 혜택 블록 (상세관 홈 탭 요약 / 혜택 탭 전체) |
