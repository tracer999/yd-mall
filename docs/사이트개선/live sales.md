...
| 비로그인 → `/checkout/choose` 경유 | ✅ `live_show_id` 유지됨 |
| 상세 50회 연속 요청 | ✅ 50/50 모두 200 |
| 기존 기능 회귀 | ✅ `/` `/best` `/deals` `/group-buy` `/coupon` `/exhibition` `/recommend` 전부 200 |

**아직 검증 못한 것 (위험 낮음, 2차에서 확인)**

- **라이브가 위에 쿠폰을 얹은 전체 결제.** 쿠폰 표시·다운로드·복귀는 확인했으나, 로그인 세션이 필요해
  "라이브가 → 쿠폰 할인 → 최종 결제"를 끝까지 통과시키지는 못했다. `checkoutController` 가 `items[].price`
  (= 라이브가) 위에 기존 쿠폰 엔진을 그대로 태우는 **공동구매와 동일한 코드 경로**라 위험은 낮다.
- **로그인 회원의 라이브 바로구매.** 비회원(guest) 경로만 통과시켰다. 역시 공동구매와 같은 분기다.
