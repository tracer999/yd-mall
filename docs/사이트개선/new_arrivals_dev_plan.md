...
| `controllers/productController.js:102-112` | THEME 분기 + `badgeMap={5:'BEST',6:'NEW'}` | **분기 전량 삭제** → `/best`·`/new` 리다이렉트로 대체 (§9-1) |
...
| 3-b | **THEME 축 폐기** | 카테고리 5·6 비활성 + `/best`·`/new` 리다이렉트, THEME 분기·테마 탭·상품폼 테마선택 제거 | §9-1. 고아 상품 0건 확인됨 |
...
**확정 — THEME 분류축 폐기 (§9-1, 데이터 확인 완료)**

조사 결과 THEME 카테고리는 **5번(베스트)·6번(신규) 단 2행**이며 둘 다 mall 1 소속이다. 그리고 셋을 확인했다.

1. **`products.theme_category_id` 컬럼은 DB 전체가 NULL 이다** — 이미 죽은 컬럼이고, 실제 연결은 전부 `product_themes` 조인 테이블(테마5: 30건 / 테마6: 18건)이 담당한다. 따라서 "`theme_category_id=6` 인 상품 정리"는 **정리할 행이 애초에 없다**.
2. **테마 6번 폐기 시 유실되는 상품은 0건이다.** 18건 중 **15건**은 `sale_start_date ← created_at` 백필만으로 100일 자동 판정에 걸리고, 남은 **3건은 전부 이미 NEW 뱃지 보유** → 강제노출로 살아남는다. 기간 밖 + 뱃지 없는 고아는 **0건**.
3. **테마 5번(베스트)도 동일한 좀비다.** `/best` 는 커밋 e48ee05 에서 이미 상품그룹 수동 큐레이션으로 옮겨갔는데, 테마 5는 아직 옛 `BEST 뱃지 OR` 로직(`productController.js:104-108` 의 `badgeMap`)을 들고 있어 `/best` 와 결과가 다르다. **6번과 같은 문제를 5번도 갖고 있다.**

→ **THEME 분류축 자체를 폐기한다.** 5·6번만 골라 없애는 게 아니라 축을 걷어낸다(THEME 행이 이 둘뿐이므로 결과적으로 같지만, `badgeMap` 하드코딩과 THEME 분기 코드가 통째로 사라지는 게 요점이다).

| 대상 | 처리 |
|---|---|
| `categories` 5·6번 | `is_active=0` **비노출**(삭제 아님 — `product_themes` FK·이력 보존, 되돌릴 수 있게) |
| `/products?category=5` · `?category=6` 접근 | 각각 `/best` · `/new` 로 **301 리다이렉트** |
| `productController.js:102-112` THEME 분기 | **전량 삭제** (`badgeMap = {5:'BEST', 6:'NEW'}` 하드코딩 포함) |
| `products.theme_category_id` 컬럼 | 전량 NULL 이므로 코드 참조만 제거(`admin/productController.js:338`, `805-808`). **컬럼 DROP 은 이번 스코프 밖** — 별도 정리 커밋으로 |
| 관리자 상품 폼의 테마 다중선택 | 제거 (`admin/productController.js:574`·`695-701` 의 `product_themes` INSERT/DELETE 포함) |
| 관리자 카테고리 화면의 **테마 탭** | 제거 (`TAB_TO_TYPE` 에서 `theme` 삭제) |
| `routes/sitemap.js:91` THEME 카테고리 수록 | 제거 (비활성 카테고리를 사이트맵에 넣지 않는다) |
| `product_themes` 테이블 | **데이터·테이블 모두 존치.** 지금 지울 이유가 없고, 되돌릴 여지를 남긴다 |

**승인 필요 (잔여 1건)**

4. **mall 2(종합관, 9,677건)의 신상품 정책.** 백필 근거가 없어 초기 0건이다. (a) 운영이 주요 상품에 판매시작일을 일괄 지정할지, (b) mall 2 는 신상품 메뉴를 당분간 숨길지 결정 필요. **답 없으면 (a) 로 진행**한다 — 메뉴는 살리되 목록이 비고, 관리자 일괄 지정으로 채우는 방식.
