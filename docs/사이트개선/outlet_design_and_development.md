...
> 최초 작성 2026-07-11 · 전면 개정 2026-07-13 · 구현 완료 2026-07-13 · **후속 완료 2026-07-14.**
> **상태: ✅ 완료 · 선택형 모듈 · 콘텐츠 게이트로 자동 노출/숨김.**
>
...
>
> 2026-07-14 에 남아 있던 §6(형제 메뉴의 빈 메뉴 문제)을 닫았다 — 콘텐츠 게이트를
> 공동구매·쇼핑라이브까지 확장하고 세 메뉴를 모두 켰다. [§8](#8-후속-2026-07-14--게이트를-형제-메뉴로-확장) 참고.
>
> **GNB 를 수동으로 끄지 않는다.** 콘텐츠 게이트(§4-5)가 콘텐츠 수를 보고 자동으로 켜고 끈다.
> 이게 "빈 메뉴" 문제의 영구 해법이다.
...
## 6. 형제 메뉴의 빈 메뉴 문제 — **해소됨 (2026-07-14)**

> 아래는 2026-07-13 시점의 기록이다. 결론은 §8 에 있다.
...
**→ 두 안 모두 채택하지 않았다.** 그 사이 공동구매·쇼핑라이브 **모듈이 실제로 구현되면서**
"모듈이 없다"는 전제가 사라졌기 때문이다(`services/groupBuy/` · `services/live/`).
모듈이 있으면 게이트로 풀 수 있다 — §4-5 가 예고한 그대로다. §8 참고.
...
- ⚠️ **몰 프리셋을 (재)적용하면 아울렛이 꺼진다.** `services/mall/mallProvisioner.js` 의 `applyFeatureMenus`
  는 `presets.js` 의 `featureMenus` 목록에 없는 GNB 메뉴를 전부 `is_enabled = 0` 으로 되쓴다.
  `OUTLET`·`GROUP_BUY`·`LIVE` 는 그 목록에 없다 — **신규 몰은 이 셋이 꺼진 채로 만들어진다**(의도된 기본값:
  콘텐츠가 없는 몰이니까). 이미 운영 중인 몰에 프리셋을 다시 적용하면 켜둔 설정이 함께 날아가므로,
  프리셋 재적용 후에는 `scripts/migrate_content_gate_menus.sql` 로 다시 켠다.

---

## 8. 후속 (2026-07-14) — 게이트를 형제 메뉴로 확장

§4-5 는 "이 게이트는 아울렛만이 아니라 **공동구매·쇼핑라이브 등 다른 콘텐츠 의존 메뉴에도 같이
적용할 수 있다**"고 적어 두고 아울렛에만 걸었다. 그 사이 두 모듈이 구현됐고, 두 서비스에는
게이트가 쓸 `hasAnyPublic(mallId)` 가 **이미 있는데 아무도 호출하지 않고 있었다.**
반쪽 방어였다 — 컨트롤러의 0건 폴백(`comingSoon`)만 있고 GNB 쪽 게이트가 없어서,
메뉴를 켜는 순간 "GNB 에는 있는데 눌러보면 준비중"이 다시 생기는 상태였다. 이번에 그 선을 이었다.

### 8-1. 산출물

| # | 파일 | 변경 |
|---|---|---|
| 1 | `services/menu/navigationService.js` | `CONTENT_GATES` 에 `GROUP_BUY`·`LIVE` 추가. 관리자 화면이 사유를 읽도록 `checkContentGate()` export |
| 2 | `controllers/admin/groupBuyController.js` | 등록·수정·삭제에 `invalidateContentGate(mallId)` |
| 3 | `controllers/admin/liveController.js` | 등록·수정·**상태 변경**·삭제에 `invalidateContentGate(mallId)` |
| 4 | `controllers/admin/menuPreviewController.js` | 제외 사유에 **"콘텐츠 부족 — 채울 내용이 없어 자동으로 숨겨짐"** 추가 |
| 5 | `scripts/migrate_content_gate_menus.sql` | mall 1·2 의 `OUTLET`·`GROUP_BUY`·`LIVE` 를 `is_enabled = 1` 로 |

### 8-2. 판정 기준이 메뉴마다 다르다

| 메뉴 | 게이트 | 왜 |
|---|---|---|
| `OUTLET` | 판매중 상품 ≥ `outlet_setting.min_product_count` (기본 30) | 상시 매장이다. 몇 개 없으면 매장 꼴이 안 난다 |
| `GROUP_BUY` | 공개 공동구매 ≥ 1건 | **한 건만 열려도 그 자체가 콘텐츠**다 |
| `LIVE` | 공개 라이브 ≥ 1건 | 〃 (방송 하나짜리 라이브관은 정상이다) |

게이트 기준은 **각 컨트롤러의 0건 폴백 기준과 같은 값**이어야 한다. 둘이 어긋나면
GNB 와 실제 페이지의 판단이 갈려 §1-1 의 빈 메뉴가 그대로 재현된다.

### 8-3. `is_enabled` 의 뜻이 명확해졌다

```text
is_enabled   = "이 몰이 이 채널을 쓰는가"    (운영자 의사)
콘텐츠 게이트  = "지금 보여줄 게 있는가"       (자동 판정)
```

둘이 분리됐으므로 **운영자는 메뉴를 켜둔 채 방치해도 된다.** 콘텐츠가 없으면 게이트가 숨기고,
생기면 30초 안에 저절로 나타난다(관리자가 등록하면 캐시를 즉시 비우므로 사실상 즉시).

### 8-4. 검증 결과 (2026-07-14, 로컬 3006)

```text
mall 1 · 건강식품관   아울렛 0건 / 공동구매 1건 / 라이브 0건
  GNB            → 공동구매만 노출                      ✅
  /outlet 직접    → 준비중 랜딩                          ✅
  /live 직접      → 준비중 랜딩                          ✅

mall 2 · 종합관      아울렛 42건 / 공동구매 1건 / 라이브 0건
  GNB            → 아울렛 + 공동구매 노출, 라이브 숨김     ✅

관리자 · 메뉴 미리보기
  아울렛(mall 1)·쇼핑라이브 → "콘텐츠 부족 — 자동으로 숨겨짐"  ✅
```

이제 `module_ready = 1` 이고 `is_enabled = 1` 인데 GNB 에 없는 메뉴는 **반드시 관리자 화면이
이유를 말해 준다.** 원인 불명으로 남는 경우가 없다.
