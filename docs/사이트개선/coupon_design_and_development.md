...
### 4-3. 무료배송 쿠폰은 1·2차 범위 밖

레퍼런스 §9 는 MVP 혜택으로 정액·정률·**무료배송**을 든다. 그러나 이 저장소에는 **배송비 개념 자체가 없다.**
...
DB                 shipping 관련 테이블 없음
```

배송비 정책(무료배송 기준금액·도서산간 추가비)을 먼저 만들지 않으면 무료배송 쿠폰은 **할인할 대상이 없다.** 배송비 모델링을 선행 과제로 분리한다.
...
### 6-1. 주문당 쿠폰 1장을 유지한다 (1·2차)

`orders.user_coupon_id` 가 단수라는 것은 우연이 아니라 **현재 서비스의 정책**이다. 여러 장을 허용하려면 `order_coupons` 조인 테이블과 §9 의 중복 정책 엔진이 함께 와야 한다. 1·2차에서는 1장 제약을 명시적으로 유지하고, 화면에도 "쿠폰은 1장만 사용할 수 있습니다"를 표기한다.
...
## 9. 중복 할인 정책 (3차 — 지금은 명문화만)

레퍼런스 §4 의 계산 순서는 현재 스키마에서 **부분적으로만** 성립한다.

```text
1. 상품 기본 할인      ✅ products.discount_rate
2. 상품 쿠폰           ❌ 없음 (scope 개념 부재)
3. 장바구니·주문 쿠폰   ✅ 1장만
4. 배송비 쿠폰         ❌ 배송비 모델 자체가 없음 (§4-3)
...
`order_items` 에 `coupon_discount_amount` 컬럼이 필요하다. **부분 취소를 지원하기 전까지는 상품 쿠폰을 도입하지 않는다.** 이것이 1·2차에서 주문 쿠폰만 유지하는 실질적 이유다.
...
- [ ] **C3** `getComplete` 의 결제 우회 차단 🔴🔴 — `isTest` 를 `req.query.test` 에서 분리, 쿠폰·포인트를 PENDING 주문 행에서만 읽기, 불필요하면 라우트 제거 (**최우선**)
- [ ] **C1** 주문 취소 시 쿠폰 복원 (`mypageController` 고객 취소 · `admin/orderController` 관리자 취소) 🔴
- [ ] **C4** `postForm` 할인 재검증에 `max_total_uses` 추가 🟠
- [ ] **C6** `views/user/mypage/coupons.ejs:28` 의 죽은 `percent` 분기 제거 🟡
- [ ] C1 검증 — 취소 후 쿠폰함에서 "사용 가능"으로 복귀, 만료된 쿠폰은 복원하되 사용 불가

> C2(RESERVED)·C5(코드 UI)·C7(삭제/상태)는 스키마가 필요하므로 1차에서 함께 처리한다.

#### 10-2-1. 1차 — 다운로드 쿠폰존 (GNB `/coupon`)

- [ ] **D1** 마이그레이션 — `coupons` ALTER(`mall_id`·`issue_method`·`status`·`download_*`·`issue_limit`·`issued_count`·`valid_days`), `user_coupons` ALTER(`issued_by` enum 확장·`expires_at`·`reserved_order_id`·`reserved_at`), `coupon_download` 테이블 생성
- [ ] **D2** 백필 + `auth.js` · `checkoutController` · `admin/couponController` 의 `coupon_type`/`is_active` 참조를 `issue_method`/`status` 로 **동시** 교체 (§4-2)
- [ ] **D3** 관리자 폼 재편 — 발급 방식별 동적 필드, 다운로드 설정(수령기간·선착순 수량)
- [ ] **D4** 관리자 목록 — 몰·발급방식·상태·수령/사용률 컬럼, 삭제 대신 `ENDED` 종료
- [ ] **D5** 다운로드 쿠폰 1건 실제 등록·발행
- [ ] **D6** 고객 쿠폰존 `/coupon` — `controllers/couponController.js`, `views/user/coupon/list.ejs`, 0건 폴백
- [ ] **D7** 수령 액션 `POST /coupon/:id/claim` — 트랜잭션 · `coupon_download` PK 로 중복 차단 · `issued_count` 선착순 `affectedRows` 판정 (§6-3)
- [ ] **D8** 쿠폰 코드 입력 UI 를 쿠폰존으로 이관 (C5) — 기존 `POST /checkout/apply-coupon-code` 재활용
- [ ] **D9** RESERVED 점유 (C2) — 주문 생성 시 점유, 결제 확정 시 USED, 취소 시 해제
- [ ] **D10** 마이페이지 쿠폰함에 "주문 진행 중" 상태 추가 · 만료임박 강조
- [ ] **D11** `routes/feature.js` 의 `comingSoon('coupon')` 교체
- [ ] **D12** 동시 요청 검증 — `issued_count` 가 `issue_limit` 을 넘지 않는지, 같은 회원 2회 수령 차단, 같은 쿠폰 2개 주문 동시 점유 차단
- [ ] **D13** 받은 쿠폰이 `/mypage/coupons` 와 체크아웃에 노출되는지 확인

#### 10-2-2. 2차 — 정률 할인 + 적용 범위

- [ ] **P1** `coupons` ALTER — `benefit_type` · `discount_rate` · `max_discount_amount` · `scope_json`
- [ ] **P2** 관리자 폼 — 정률 입력 + **최대 할인액 필수 검증**
- [ ] **P3** 할인 계산기 분리 — `services/coupon/discountCalculator.js` (정액/정률 + 최대할인 상한 + 절사)
- [ ] **P4** `scope_json` 적용 — 카테고리·브랜드·뱃지 포함/제외 판정
- [ ] **P5** 쿠폰함·체크아웃·쿠폰존에 정률 표시 (C6 의 `percent` 분기 부활)
- [ ] **P6** 검증 — 정률 쿠폰이 최대 할인액을 넘지 않는지, 제외 대상 상품만 담긴 장바구니에서 적용 불가 판정

#### 10-2-3. 3차 — 유보 (조건 충족 후)

- [ ] 배송비 모델링 → 무료배송 쿠폰 (§4-3)
- [ ] `order_items.coupon_discount_amount` → 부분 취소 배분 (§9)
- [ ] 상품 쿠폰(`PRODUCT` scope) — 부분 취소 선행
- [ ] 중복 정책 (`combination_group` · `stackable` · `priority`) + `order_coupons` 다중 적용
- [ ] `coupon_target` · `coupon_issue_condition` 분리 (§4-1 의 조건 충족 시)
- [ ] 발급 조건 확장 (등급·누적구매·생일·휴면) — **`user_grade` 선행** (gnb §8-5)
- [ ] PENDING 점유 해제 배치 (30분 초과)
- [ ] 관리자 쿠폰 회수(`REVOKED`) · 변경 이력
