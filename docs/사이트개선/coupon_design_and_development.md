...
- [ ] **P7** `benefit_type` 에 `SHIPPING_FREE` · `SHIPPING_FIXED` 추가
- [ ] **P8** `orders.shipping_coupon_id` — **주문 쿠폰 1장 + 배송비 쿠폰 1장** 동시 적용 (§6-1)
- [ ] **P9** `shipping_discount ≤ shipping_fee` 상한 강제
- [ ] **P10** 쿠폰존·쿠폰함·체크아웃에 배송비 쿠폰 표시
- [ ] **P11** 검증 — 무료배송 쿠폰이 배송비를 초과 할인하지 않는지, 할인 쿠폰과 배송비 쿠폰 동시 적용
