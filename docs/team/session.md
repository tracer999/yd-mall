...
**관리자 메뉴를 추가할 때는 반드시 `코드 배포 → 마이그레이션` 순서로 한다.**
`middleware/adminMenu.js` 는 매 요청 DB 를 읽어 사이드바를 그리고 **라우트 존재 여부는 확인하지 않는다.**
dev·prod 가 같은 DB 를 보므로, 마이그레이션을 먼저 돌리면 운영 사이드바에 링크가 뜨는데
클릭하면 404 인 구간이 열린다. (이번 세션에서 A2/B4/B5 때 한 번 저질렀고 푸시로 치유했다.
B6/B7 과 테마/FAQ 는 순서를 지켜 창이 열리지 않았다.)

또한 dev 와 prod 가 같은 DB 이므로 **검증용 POST/INSERT 전에 스냅샷을 뜨고 끝나면 원복 후 대조**한다.
