...
### ⚠️ 로컬 앱 좀비 주의 (PM2)
로컬(WSL)에 PM2 로 `dev-mall` 이 등록돼 있으면, `node app.js` 를 kill 해도 **PM2 가 즉시 재살려** 포트 3006 을 옛 코드로 잡는다. 그러면 새 라우트가 404 로 보인다(이번 세션에서 겪음). 증상 시:
`pm2 delete dev-mall` → 포트 확인(`ss -ltnp | grep 3006`) → `node app.js` 재기동. (운영 PM2 는 별 서버라 무관.)
