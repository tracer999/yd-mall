...
- **뷰:** `views/admin/categories/list.ejs` (탭 + 트리 목록 + 등록 모달 + 행별 인라인 수정 폼)

카테고리는 **type(NORMAL / THEME / BRAND)** 이라는 병렬 분류축과, 각 type 안의 **`parent_id` 자기참조 계층(최대 3뎁스)** 두 축으로 관리합니다. 화면은 3개 탭(상품 / 테마 / 브랜드)으로 나뉘고, 각 탭은 뎁스별 아코디언 트리로 렌더합니다. 편집 대상 몰(`req.adminMallId`, `middleware/adminMallContext.js`)의 카테고리만 조회·생성됩니다.
...
| 메서드 | URL | 핸들러 | 설명 |
|--------|-----|--------|------|
| GET | `/admin/categories` | getList | 탭별 트리 목록 + 등록 모달 표시 (`?tab=`, `?page=`, `?error=`) |
...
- 실패는 예외를 던지지 않고 `/admin/categories?tab=<탭>&error=<메시지>` 로 리다이렉트해 화면 상단 배너로 표시합니다.

---

## 3. 목록 조회 (GET /admin/categories)

- **쿼리:** `SELECT * FROM categories WHERE mall_id = ? ORDER BY display_order ASC, id ASC`  
- **상품수:** `products` 를 `category_id` 로 GROUP BY 하여 카테고리별 상품수(`productCount`)를 붙입니다(같은 몰 한정).  
- **트리 구성:** type 별로 `flattenTree()` 가 부모 → 자식 순으로 평탄화하며 `_depth`(1부터)를 부여합니다. 정렬 기준은 `display_order`, 동률은 `id`.  
- **뷰:** 상단 3탭(상품/테마/브랜드) + 우측 상단 "카테고리 등록" 모달. 세 탭 모두 서버에서 렌더되고, JS 로 전환합니다.  
- **테이블 컬럼:** 순서(display_order) · (로고/이미지 — NORMAL·BRAND만) · 이름 · (간략 설명 — NORMAL 1뎁스만) · 상위 카테고리 · 노출(is_active / pc_visible / mobile_visible) · 상품수 · 관리(수정/삭제)  
- **수정:** 각 행이 자체 `/admin/categories/edit` POST 폼(multipart)이며 `form="cat-form-<id>"` 로 셀 전반의 input 을 묶습니다.  
- **삭제:** 하위 카테고리가 있으면 버튼이 비활성. 없으면 확인 후 hidden form `#deleteForm` → `/admin/categories/delete` POST (id, active_tab).

### 3.1 페이지네이션

- 아코디언 정합성을 위해 **행이 아니라 "최상위(1뎁스) + 그 서브트리 전체"** 를 한 단위로 잘라 페이지를 나눕니다. 서브트리가 페이지 경계에서 쪼개지지 않습니다.  
- `TOP_PER_PAGE = 100` (최상위 기준). `pageInfo[type] = { page, totalPages, total, perPage }` 의 `total` 은 최상위 개수입니다.  
- `?page=` 는 **활성 탭에만** 적용되고 나머지 탭은 1페이지로 렌더합니다.

### 3.2 상위 카테고리 선택지 (addParentOptions)

- type 당 1벌만 만들어 뷰에 넘기고, 클라이언트가 select focus 시점에 자기/후손을 걸러 씁니다(노드마다 만들면 O(n³) 이 되어 브랜드 1354개 몰에서 응답이 터졌습니다).  
- 후보는 `depth <= maxDepth - 1` 인 노드뿐입니다. 자기/후손 제외는 UX 편의이고, 실제 방어는 서버의 `wouldCreateCycle` / `assertDepthAllowed` 가 합니다.

---

## 4. 카테고리 추가 (POST /admin/categories/add)

- **파라미터:** `name` (필수), `type` (NORMAL/THEME/BRAND, 그 외는 NORMAL), `active_tab`, `parent_id` (없으면 최상위), `display_order` (비우면 같은 type·몰의 MAX+1), `description`, `logo_image` (파일), `is_active` / `pc_visible` / `mobile_visible` (기본 1)  
- **검증 순서:**
...
- **성공 시:** `res.redirect('/admin/categories?tab=<active_tab>')`
...
- **파라미터:** `id`, `name`, `display_order`, `type`, `active_tab`, `parent_id`, `description`, `existing_logo` (기존 로고 유지), `logo_image` (새 파일), `is_active` / `pc_visible` / `mobile_visible`  
...
- **성공 시:** `/admin/categories?tab=<active_tab>` 로 리다이렉트  
