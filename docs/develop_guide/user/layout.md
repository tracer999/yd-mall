...
- **구성 partial:**
  - `views/partials/storefront/header.ejs` — Top Bar + 로고/검색/유저액션 + 모바일 GNB 슬라이더 + PC GNB
...
- 하단 바는 4칸 그리드(h-16 + safe-area). 항목은 **하드코딩**입니다(`feature_menu.mobile_quick` 행이 0건).
- 비로그인이면 4번째 칸이 "로그인"(`/auth/login`), 로그인이면 "마이"(`/mypage`, 프로필 이미지 표시).
- **카테고리 전체 레이어**(`#yd-cat-layer`, z-60): 탭으로 여는 **단계별 드릴다운**.
  - 1뎁스 탭 → 2뎁스 패널, 2뎁스 탭 → 3뎁스 패널. 자식이 없으면 바로 상품목록(`/products?categoryId={id}`)으로 이동.
